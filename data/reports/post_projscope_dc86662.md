...
**Label:** post_projscope | **Date:** 2026-08-06T14:51:18.178276700+00:00 | **Commit:** dc86662 | **Model:** BGESmallENV15_384
**Completed tracks:** r4fix
...
| implementation | 29 | 0.53 | 0.58 | 0.58 | 62% |
| overview | 33 | 0.67 | 0.77 | 0.81 | 67% |
...
|----|-------|----------|-----|
| hero-chat-endpoint | "How does the chat endpoint work?" | handlers.rs, chat | code:send_chat (0%), code:PromptMessages (0%), code:send_chat_rag_only (0%) |
...
| impl-app-state | "How is application state initialized?" | state.rs, AppState | code:initialize (5%), code:state (0%), code:state (0%) |
...
| rel-error-handling | "How do errors propagate through the system?" | error.rs | code:gemini_retry_on_transient (17%), file:caravan/rpc/rust/caravan-rpc/src/errors.rs (0%), code:oc (0%) |
...
| edge-ambiguous | "Tell me about the store" | vector_store.rs | folder:code-rag/src/store (0%), code:Bell (0%), code:Store (0%) |
...
| b5-sig-query | "Functions that return Result<T, Error>" | retriever.rs, retrieve | code:as (98%), code:u (97%), code:d (97%) |
| b5-body-query | "Which function parses JSON configs?" | from_json_str | code:Qs (99%), code:LoadConfig (98%), code:parseProviderForm (98%) |
| a3-api-folder | "What's in the src/api folder?" | code-rag/src/api | folder:cioport/web/src/api (100%), folder:cioport/web/src (100%), folder:concurrens/server/modules/api/src (100%) |
| a4-retriever-file | "What does retriever.rs do?" | code-rag/src/engine/retriever.rs | code:FileChunk (89%), file:code-rag/src/harness/matching.rs (62%), code:matches_file (43%) |
