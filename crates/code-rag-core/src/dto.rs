...
/// Source reference in a chat response. Carries chunk-type discriminator,
/// stable chunk id, and display fields. Used by both the HTTP `/chat`
/// response builder and the MCP tool result formatter, hence its home
/// in the shared core crate.
///
/// `Deserialize` derive is here so non-HTTP callers (test harnesses,
/// future SDK consumers) can roundtrip the source list through JSON.
