...
### Release packaging (Phase 6)

After in-session use confirmed the MCP works, packaged it for external adoption. The path to "this is installation" took three full passes — each one bounced off a hard pushback from the user — and ended at: **download one exe, edit one YAML, run the exe, done**. No console commands.

**The arc.** First draft was `code-rag-mcp init [PATH]` — a clap subcommand that wrote the skill + `.mcp.json` + `.gitignore` into a target dir, idempotent, jq-style merge for `.mcp.json`. Worked fine. User pushback: even one console command is too much. Second draft pivoted to two GitHub Action workflow templates that users would drop into their own repos' `.github/workflows/`, trigger from the Actions tab, then merge a PR — the "drop in any repo or any 'parent folder of many repos'" framing. Built it: two templates (single-repo + workspace) embedding the skill verbatim as YAML heredocs, jq-merging `.mcp.json`, opening a PR via `peter-evans/create-pull-request@v6`. User pushback: "you expect users to run gh workflow of their own? That's not 'installation'." Both drafts deleted; templates dir removed entirely. Third draft is what shipped.

**What shipped.**

1. **One binary, not two.** Promoted `code-raptor`'s ingest orchestration (`run_full_ingestion`, `run_incremental_ingestion`, all helpers) out of its `main.rs` into a new public lib module [crates/code-raptor/src/orchestrate.rs](crates/code-raptor/src/orchestrate.rs) exposing `pub async fn ingest_repo(opts: IngestOpts)`. `code-raptor`'s binary is now a thin shell that just unwraps the CLI args and calls into the lib. Added an `ingest` subcommand to `code-rag-mcp` that takes the same args and calls the same lib function. Released zip ships only `code-rag-mcp.exe` — the standalone `code-raptor` binary still exists as a workspace dev/harness tool but isn't part of the user-facing release. The `code_rag_reindex` MCP tool now spawns `std::env::current_exe()` (recursive self-spawn into the `ingest` subcommand) instead of an external `code-raptor`, so deployments don't have to put two binaries on PATH.

2. **Bare-run YAML setup.** `code-rag-mcp` with no args (the "double-click the exe" case, detected pre-clap via `std::env::args().len() == 1`) reads `code-rag-mcp.config.yaml` from `current_exe().parent()` (the dir containing the exe — works wherever the user extracts the zip). On first run with no config, writes a default template there and exits with instructions. On subsequent runs, parses `target_path: ...` and `workspace: bool`, then writes `.claude/skills/code-rag.md` (skill embedded into the binary at compile time via `include_str!("../skills/code-rag.md")`), merges a `code-rag` entry into `.mcp.json` under `mcpServers` (preserving any existing servers via `serde_json::Value` round-trip), and appends `.code-rag-mcp/` to `.gitignore` (idempotent). The bare-run path is "no console" by user contract — works equally well from a double-click or an empty `code-rag-mcp` invocation in a terminal.

3. **`.code-rag-mcp/` not `.code-rag/`.** Renamed the index dir per user direction — clearer namespace alongside the binary name, less chance of colliding with anyone else's `.code-rag/`. Default in `--db-path` is `./.code-rag-mcp/index.lance`. The setup mode hardcodes this entry into the generated `.mcp.json` and `.gitignore`.

4. **Agent-driven first ingest.** Skill rewritten so that "the index is missing or empty" is the agent's signal to call `code_rag_reindex mode=full`, not a "tell the user to run …" hand-off. Empty-store smoke confirmed the MCP starts cleanly against a fresh repo (LanceDB tolerates missing tables; `code_rag_search` returns `{intent, hits: []}` rather than panicking). Combined with bare-run setup and one-binary install, this means the user's full install experience is: download → edit YAML → run exe → open Claude Code → ask a question. The first ingest happens transparently inside the agent's first MCP call.

5. **Manually-triggered release workflow.** [.github/workflows/release.yml](.github/workflows/release.yml) is `workflow_dispatch` with a `version` input. Builds the single `code-rag-mcp` binary across the matrix `x86_64-unknown-linux-gnu` / `aarch64-apple-darwin` / `x86_64-pc-windows-msvc` on GitHub-hosted runners, stages it alongside the YAML config template + a generated `README.txt`, zips, computes SHA256, and uploads to a draft GitHub Release. Operator publishes after smoke-testing. The "release is manual" contract sits at the trigger; the cross-platform build itself is automated because GitHub-hosted runners are the path of least resistance.

**Reranker model bundling.** Decided against. The `cross-encoder/ms-marco-MiniLM-L-6-v2` ONNX is 88 MB on disk; bundling would 3x the release zip size for everyone, including users who already have it cached. Auto-download on first `code_rag_reindex` (cached at `~/.cache/huggingface/hub/`) handles 95% of users; the rest set `CODE_RAG_RERANKER_DIR` (Phase 4 hook) or `--no-rerank`. Documented in the release notes + `README.txt` template.

**One earlier follow-up item is now closed.** "`code_rag_reindex(mode?)` parameter — code currently no-args, full-only" — the other session implementing the incremental-ingest hardening already wired the `mode` parameter (see "Incremental ingest hardening" subsection above). Phase 6 just folded the new contract into the skill + README; no code change to the tool itself.

**Verification.** End-to-end smoke against a clean dir on Windows: extracted zip, edited `code-rag-mcp.config.yaml` to point at a scratch repo, double-click invoked the bare-run setup, all three files (`.claude/skills/code-rag.md`, `.mcp.json`, `.gitignore` entry) appeared with correct contents. The `ingest` subcommand smoke (`code-rag-mcp ingest <repo> --db-path X --single-repo --full`) parsed and stored 535 code + 2 readme + 7 crate + 3 module_doc chunks and resolved 703 call edges — bit-for-bit equivalent to running the standalone `code-raptor ingest` did. Workspace tests stay green.
...
- **Skill text drift between MCP crate and workflow YAML.** The two workflow templates embed the skill verbatim as heredocs. If [crates/code-rag-mcp/skills/code-rag.md](crates/code-rag-mcp/skills/code-rag.md) is edited without re-syncing the templates, users get stale guidance. Documented as a manual step in [docs/release.md](docs/release.md); a CI lint that fails when they diverge is the long-term fix.
